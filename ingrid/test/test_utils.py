...
class TestUtils(unittest.TestCase):
    ...
    def test_associate_successors(self):
        ...
        succ_dict = {
            "successors": [
                {"source": "zero", "target": "one", "edge_attribute": "type"},
                {"source": "zero", "target": "two", "edge_attribute": "type"},
            ]
        }
    ...
    def test_associate_predecessors(self):
        ...
        pred_dict = {
            "predecessors": [
                {"target": "zero", "source": "one", "edge_attribute": "type"},
                {"target": "zero", "source": "two", "edge_attribute": "type"},
            ]
        }
    ...
    def test_associate_node_types_settings(self):
        ...
        car_settings = associate_node_types_settings(df, tr, {}, node="car")
        car_expect = {
    ...
    def test_associate_renames(self):
        ...
        changed_names = [
            "A_atomic thing_composite thing-end1",
        ...
        for k, v in name_id_dict.items():
            expect_dict.append({"original_name": k, "original_id": v})
    ...
    def test_build_dict(self):
        ...
        expect = {"id": 1, "name": "Car"}
        self.assertDictEqual(expect, built_dict)

    def test_make_object(self):
        obj_attrs = {"name": "Car", "id": "123"}
    ...
    def test_create_column_values_space(self):
        ...
        expect_space = ["car qua chassis context", "wheel qua hub context"]
    ...
    def test_create_column_values_singleton(self):
        ...
        created_cols = create_column_values_singleton(
            first_node_data=first_node_data, second_node_data=second_node_data
        )
    ...
    def test_get_node_types_attrs(self):
        ...
        attr_list = [
            {"Notes": "little car to big Car", "Two such Cols": 1},
            {"Notes": 2, "Two such Cols": 3},
        ]
        # Try with multiple notes cols and a node with two types.
        self.assertEqual({"Atomic Thing"}, node_type_cols)
        self.assertListEqual(attr_list, node_attr_dict)
    ...
    def test_match_changes(self):
        ...
        for key in pairings.keys():
            if key in unstable_keys:
                ...
                pairings_str.update(
                    {
                        key.named_edge_triple: pairings[key][
                            0
                        ].named_edge_triple
                    }
                )
    ...
    def test_match(self):
        ...
        match_edge2 = DiEdge(
            source=wheel, target=engine, edge_attribute="owner"
        )
        ...
        vehicle = Vertex(
            name="Vehicle", id="4", original_id="1", original_name="Car"
        )
        rename_edge = DiEdge(
            source=vehicle, target=engine, edge_attribute="owner"
        )
    ...
    def test_get_setting_node_name_from_df(self):
        ...
        df = pd.DataFrame(data=data_dict)
        setting_node = get_setting_node_name_from_df(
            df=df, column="Atomic Thing", node="wheel"
        )
    ...
    def test_to_excel_df(self):
        ...
        excel_df = pd.DataFrame(
            data=dict([(k, pd.Series(v)) for k, v in excel_data.items()])
        )
    ...
    def test_to_uml_json_decorations(self):
        ...
        expect = {
            "id": "new 0",
            "ops": [
                {"op": "decoration", "path": "/m2/home", "value": "volume"}
            ],
        }
    ...
    def test_to_uml_json_edge(self):
        ...
        expect = {
            "id": "nal3013",
            "ops": [
                {"op": "O P", "path": "/m2/" + "connector", "value": None}
            ],
        }
