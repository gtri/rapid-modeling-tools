...
def main():
    settings_msg = (
        "Subparser example demonstating bundled configurations "
    ...
    curl_parser.add_argument(
        "--user-agent", help="Specify the User-Agent string "
    )
    curl_parser.add_argument(
    ...
    siege_parser = subs.add_parser(
        "siege",
        help="Siege is an http/https regression testing and benchmarking utility",
    )
    ...
    siege_parser.add_argument(
        "--message", help="mark the log file with a separator", type=int
    )

    # ########################################################
    ffmpeg_parser = subs.add_parser(
        "ffmpeg",
        help="A complete, cross-platform solution to record, convert and stream audio and video",
    )
    ...
    ffmpeg_parser.add_argument(
        "--fps", help="set frame rate (default = 25)", type=str
    )
    ffmpeg_parser.add_argument(
        "--size",
    ...
    ffmpeg_parser.add_argument(
        "--bufsize", help="set ratecontrol buffere size (in kbit)", type=str
    )
